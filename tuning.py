...
def configure_model(model_name, use_quantization=False, use_remote_code=False):
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map=device_map,  # Phân bổ tự động
...
def train_model(model, tokenizer, dataset, output_dir, device):
    try:
        ...
        # Thiết lập fp16 và bf16 dựa trên thiết bị
        if device == "mps":
            fp16 = False
            bf16 = False
        else:
            fp16 = True  # Hoặc thiết lập theo nhu cầu của bạn
            bf16 = False  # Hoặc True nếu GPU hỗ trợ
        ...
        training_args = TrainingArguments(
            ...
            # max_steps=1000,  # Số bước huấn luyện, thay vì num_train_epochs
            num_train_epochs=5, # tăng nếu dataset lớn hơn hoặc nếu muốn hiệu quả huấn luyện cao hơn. giúp mô hình học tập trung hơn vào dữ liệu huấn luyện
            ...
            logging_strategy="no",  # Log thông tin sau mỗi số bước
            learning_rate=2e-5,
            ...
            weight_decay=0.01 # weight_decay là một kỹ thuật giảm thiểu overfitting và điều chỉnh mô hình, giúp cải thiện hiệu suất tổng thể và ngăn chặn việc học các trọng số quá lớn.
        )
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time  # Thời gian huấn luyện tính bằng giây

        # In ra thời gian huấn luyện
        print(f"Training completed in {elapsed_time // 60} minutes and {elapsed_time % 60} seconds.")
        logger.info(f"Training completed in {elapsed_time // 60} minutes and {elapsed_time % 60} seconds.")
...
def test_model_response(prompt, max_length=1500):
    try:
        ...
        # Chuyển inputs đến thiết bị phù hợp nếu cần
        if device in ["cuda", "mps"]:
