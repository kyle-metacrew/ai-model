...
def generate_text(prompt, max_length=100):
    ...
    # inputs = {key: value.to(device).to(torch.float32) for key, value in inputs.items()}
...
# Chia dataset thành train và test (giả sử dataset có 1000 dòng)
train_size = int(0.8 * 1000)  # 80%
...
dataset["train"] = dataset["train"].shuffle(seed=65).select(range(min(train_size, 10)))
dataset["test"] = dataset["test"].shuffle(seed=65).select(range(min(test_size, 2)))
...
training_arguments = TrainingArguments(
    ...
    num_train_epochs=1, # tăng nếu dataset lớn hơn hoặc nếu muốn hiệu quả huấn luyện cao hơn. giúp mô hình học tập trung hơn vào dữ liệu huấn luyện
    eval_strategy="steps",
    eval_steps=0.2,
    logging_steps=1,
    warmup_steps=500, # Warmup có thể giúp mô hình khởi động tốt hơn
    logging_strategy="steps",
    learning_rate=5e-4, # Tăng learning_rate để mô hình học nhanh hơn trên dataset nhỏ.
...
def test_model_response(prompt, max_length=100):
    device = "cpu"
    model.to(device)
